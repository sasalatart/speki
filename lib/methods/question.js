Meteor.methods({
  addQuestion: (courseID, text) => {
    ...
    return { header: new Date(), text: 'Pregunta agregada.' }
  },
  updateQuestion: (questionID, text) => {
    ...
    return { header: new Date(), text: 'Pregunta editada.' }
  },
  removeQuestion: questionID => {
    ...
    return { header: new Date(), text: 'Pregunta eliminada.' }
  },
  toggleSubscription: questionID => {
    ...
    if (subscriptionIndex === -1) {
      ...
      return { header: new Date(), text: 'Suscrito para recibir notificaciones de la pregunta.' }
    } else {
      Questions.update({ _id: questionID }, { $pull: { subscribers: Meteor.userId() } });
      return { header: new Date(), text: 'Ya no estás suscrito para recibir notificaciones de la pregunta.' }
    }
  }
});
